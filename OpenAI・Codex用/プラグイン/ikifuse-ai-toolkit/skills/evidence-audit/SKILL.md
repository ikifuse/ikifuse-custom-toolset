...
過去のエージェントセッションに、関係する意図、決定、試行、由来が含まれる可能性がある場合は、`ctx-agent-history-search`など利用可能な履歴検索機能を使用します。最初に利用可能な状態か確認します。検索結果の抜粋だけに依存せず、対象を絞ったeventまたはsessionを確認します。index作成の失敗、古い記録、開けない元資料、除外した資料を報告します。

現在または変更され得るOpenAI製品の動作については、OpenAI公式文書の機能を使用し、取得したページを引用します。履歴や記憶を現在の仕様の代わりにしません。
