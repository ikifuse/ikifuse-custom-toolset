...
description: 外部公開の直前に実際の送信対象を検査し、外部へ出して安全かを分類します。git commit・push、pull request、GitHub Issue、外部サービスへの送信、公開物の生成、ログ・設定ファイルの共有前に使用します。特にAPIキー、token、password、private key、.envの秘密値、credential、個人情報、Cookie、session、ユーザー固有パス、ローカル専用設定を確認します。外部公開経路のない通常のローカル編集や単純な読み取り専用確認には使用しません。
---
...
- ローカル専用設定、machine設定、内部endpoint、GitHubへ入れるべきでないファイル
...
対象がローカルtextファイルの場合は、同梱scannerを実行します。
...
scannerは検出値そのものを表示せず、候補の種類、パス、行、理由を報告します。ファイルを変更しません。不存在を証明できず、すべての氏名・住所を確実に識別できず、アクセスできない外部内容を検査できず、文脈による確認の代わりにはなりません。
